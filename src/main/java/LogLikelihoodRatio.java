...
/**
     ...
     * where N = sum x_i
     *
     * If the x's sum to 1, then this is the same as the normal expression.
     * Leaving this un-normalized makes working with counts and computing the
     * LLR easier.
     *
    ...
    */
/**
     * Merely an optimization for the common two argument case of
     * {@link #entropy(long...)}
     *
    ...
    */
/**
     * Merely an optimization for the common four argument case of
     * {@link #entropy(long...)}
     *
     * @see #logLikelihoodRatio(long, long, long, long)
     *//*
    ...
    */
/**
     * Calculates the Raw Log-likelihood ratio for two events, call them A and
     * B. Then we have:
     * <p/>
     ...
     * <td>&nbsp;</td>
     * <td>Event A</td>
     * <td>Everything but A</td>
     * </tr>
     * <tr>
     * <td>Event B</td>
     * <td>A and B together (k_11)</td>
     * <td>B, but not A (k_12)</td>
     * </tr>
     * <tr>
     * <td>Everything but B</td>
     * <td>A without B (k_21)</td>
     * <td>Neither A nor B (k_22)</td>
     * </tr>
     * </tbody>
     * </table>
     *
     * @param k11
     *            The number of times the two events occurred together
     * @param k12
     *            The number of times the second event occurred WITHOUT the
     *            first event
     * @param k21
     *            The number of times the first event occurred WITHOUT the
     *            second event
     * @param k22
     *            The number of times something else occurred (i.e. was neither
     *            of these events
     ...
     *         .html for the table and the descriptions.
     *//*
    ...
    */
/**
     * Calculates the root log-likelihood ratio for two events. See
     * {@link #logLikelihoodRatio(long, long, long, long)}.
     *
     * @param k11
     *            The number of times the two events occurred together
     * @param k12
     ...
     *         http://tdunning.blogspot
     *         .com/2008/03/surprise-and-coincidence.html
     *//*
...
/**
     ...
     * over-represented in the first set.
     *
     * @param a
     *            The first counts.
     * @param b
    ...
    public static <T> List<ScoredItem<T>> compareFrequencies(Multiset<T> a, Multiset<T> b) {
        int c1 = a.size();
        int c2 = b.size();
        ...
        Queue<ScoredItem<T>> best = new PriorityQueue<ScoredItem<T>>(101, byScoreAscending);

        for (T t : a.elementSet()) {
            compareAndAdd(a, b, c1, c2, best, t);
        }
    ...
    private static <T> void compareAndAdd(Multiset<T> a, Multiset<T> b,int c1, int c2, Queue<ScoredItem<T>> best, T t) {
        int c12 = a.count(t);
        int kB = b.count(t);
        double score = rootLogLikelihoodRatio(c12, c1 - c12, kB, c2 - kB);
        if (score >= threshold) {
